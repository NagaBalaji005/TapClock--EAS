...
Copy .env.example to .env in both backend/ and frontend/ and fill values.
...
# Either DATABASE_URL (recommended) or individual DB params
...
Security: never commit .env with secrets to GitHub. Keep .env.example only.
...
Run the seed script to create sample users and attendance (if included):

cd backend
npm run seed


Default sample credentials (for local testing):
...
Password with spaces: avoid spaces or URL-encode them in DATABASE_URL.

Port conflict: change PORT in backend/.env.

CORS: ensure CORS_ORIGIN matches frontend URL.
